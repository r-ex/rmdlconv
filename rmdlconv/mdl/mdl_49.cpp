...
void ConvertAttachments_49(mstudioattachment_t* pOldAttachments, int numAttachments)
{
...
void ConvertMDLData_49(char* buf, const std::string& filePath)
{
	...
	ConvertAttachments_49((mstudioattachment_t*)input.getPtr(), oldHeader->numlocalattachments);
	...
	ConvertSrcBoneTransforms(pSrcBoneTransforms, oldHeader->pStudioHdr2()->numsrcbonetransform);
	...
	if (oldHeader->pStudioHdr2()->linearboneindex && oldHeader->numbones > 1)
	{
		...
		ConvertLinearBoneTable(pLinearBones, (char*)pLinearBones + sizeof(mstudiolinearbone_t));
	}
